...
contract Stake is Owned {
    ...
    address public XFIM = 0xebdf6b2080bd8487804abd662e2d142daa943451;
    ...
    function STAKE(uint256 tokens) external {
        require(IERC20(XFIM).transferFrom(msg.sender, address(this), tokens), "Tokens cannot be transferred from user account");
    ...
    function ADDFUNDS(uint256 tokens) external {
        require(IERC20(XFIM).transferFrom(msg.sender, address(this), tokens), "Tokens cannot be transferred from funder account");
        _addPayout(tokens);
    }
    ...
    function CLAIMREWARD() public {
        if(totalDividends > stakers[msg.sender].fromTotalDividend){
            ...
            require(IERC20(XFIM).transfer(msg.sender,owing), "ERROR: error in sending reward from contract");
    ...
    // @param _staker the address of the staker
    // ------------------------------------------------------------------------
    function yourStakedXFIM(address staker) external view returns(uint256 stakedXFIM){
        return stakers[staker].stakedTokens;
    }
    
    // ------------------------------------------------------------------------
    // Get the XFIM balance of the token holder
    // @param user the address of the token holder
    // ------------------------------------------------------------------------
    function yourXFIMBalance(address user) external view returns(uint256 XFIMBalance){
        return IERC20(XFIM).balanceOf(user);
    }
}
